...
def scrape_movie_details(movieurl,movieName):
    ...
    bio = data.find("div", id="movieSynopsis" ,class_="movie_synopsis clamp clamp-6 js-clamp").get_text().strip()
    dict1["Bio"] = bio

  
    movie_name = data.find("h1", slot="title" ,class_="scoreboard__title").get_text()
    ...
    for i in mainDiv:
        ...
        if "\nRating" in b:
            ...
            while i < len(s):
                if s[i] == "h" or s[i] == "m" or s[i] == " " or i == 0:
                    i += 1
                    continue
